...
int msams_core(const Tdata *x,int d,int N,landmarkStruct<Tdata,Tint> &lminfo,int q,outputStruct<Tdata,Tint> &out,const optionStruct &ops)
{
  ...
  if (q == 1 || ops.n_threads == 1) {
    ...
    for (i = 0; i < ops.n_threads; i++) {
      ...
      //MODIF HERE
      if (tdcopies[i].probePtIndex_end < q)
          tdcopies[i].probePtIndex_end = tdcopies[i].probePtIndex_end + 1;
      //END MODIF HERE
...
void msams_core_work(thread_data_type<Tdata,Tint> *td)
{
  ...
  int closestIndex,closestIndexOld;
  ...
  for (i = td->probePtIndex_start; i < td->probePtIndex_end; i++)
    td->out->closestLandmark[i] = (Tint) pthread_self();
  */
  ...
  for (probeIndex = td->probePtIndex_start; probeIndex < td->probePtIndex_end; probeIndex++) {
    ...
    for (i = 0; i < d; i++)
      ytrajtmp[i] = thisY[i];
    // We'll abort when the closest landmark (or data point, when
    ...
    while (!isdone) {
      ...
      if (!td->ops->probePointsAreDataPoints)
	// Find the closest landmark
	closestIndex = lm_nbrs.closest_landmark();
      else
	// Find the closest data point
	closestIndex = lm_nbrs.current_point()->index;
      // See whether the closest landmark/point changed
      if (td->ops->leapfrog && closestIndexOld != closestIndex && closestIndexOld >= 0)
...
//	assert(msams_criterion_met == true);
	if (td->ops->backtrack)
	  for (i = 0; i < d; i++)
	...
	// different landmark is done above (and will be done again on the
	// next iteration).  Here we have to test for cycling, i.e.,
	// revisiting the same sequence of positions.
    ...
    td->out->closestLandmark[probeIndex] = closestIndex + td->lminfo->index_offset;
